...
// Verifica di sola lettura: cerca il contatto su PerfectGym e riporta lo stato
// del contratto, SENZA creare nulla. Serve al form eventi del sito, che chiede
// l'email come primo passo per capire se chi prenota è socio: a quel punto la
// persona non ha ancora dato nessun consenso, quindi creare un lead come fa
// sincronizzaPgm sarebbe un trattamento dati non autorizzato (e riempirebbe
// PerfectGym di lead per ogni email digitata per prova).
//
// "Socio" = contratto CURRENT: è lo stesso criterio con cui il workflow n8n
// "tca-verifica-iscritto" risponde `iscritto` al form contatti. Un contratto
// scaduto, sospeso o non ancora iniziato NON dà la quota socio.
export type VerificaSocio = {
  socio: boolean
  esito: EsitoPgm | null
  pgmMemberId: string | null
  pgmProfileUrl: string | null
...
export async function verificaSocioPgm(email: string): Promise<VerificaSocio> {
  const vuoto: VerificaSocio = {
    socio: false,
    esito: null,
    pgmMemberId: null,
    pgmProfileUrl: null,
  ...
  try {
    const membro = await cercaMemberPgm(email)
    if (!membro) return { ...vuoto, esito: 'NUOVO' }

    const esito = classificaContrattoPgm(membro.contractStatus)
    return {
      socio: esito === 'CURRENT',
      esito,
      pgmMemberId: membro.id,
      pgmProfileUrl: urlProfiloPgm(membro.id),
      nome: membro.firstName,
      cognome: membro.lastName,
      cellulare: membro.phoneNumber,
      errore: null,
    }
  } catch (e) {
    return { ...vuoto, errore: e instanceof Error ? e.message : 'Errore PerfectGym' }
